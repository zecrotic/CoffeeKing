...
const int creamDigits[] = {29, 31, 42, 41, 44, 24, 22};
...
const int sugarDigits[] = {30, 32, 39, 38, 40, 28, 26};
...
int turnOffLeds = 2000; // how many times through the loop until the LEDs are turned off
...
void setup() {
  ...
  */
  ...
  //displayCreamDigit(buttonPushCounter);
  //displaySugarDigit(sugarButtonPushCounter);
  displayDigit(buttonPushCounter, creamDigits[7]);
  displayDigit(sugarButtonPushCounter, sugarDigits[7]);
}
...
void loop() {
  ...
  */
  //loopCounter++; // UNCOMMENT TO ENABLE SLEEP FUNCTION
  /*
  if (loopCounter > turnOffLeds) {
    loopCounter = turnOffLeds;
    displayCreamDigit(10); // turns cream digit off
  ...
  */
  ...
  i = 100;//SO I DON'T HAVE TO KEEP HOLDING DOWN THE WEIGHT SENSOR
  if (startState == LOW && i > 50) {
    turnOnLeds(); // I want the LEDS to turn on then start making coffee but this does it inthe opposite order
    delay(10);
    if (loopCounter == 0)
