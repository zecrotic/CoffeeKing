...
void loop() {
  ...
  if (sugarButtonState != sugarLastButtonState) { // if the state has changed, increment the counter
  ...
  if (sugarDecrementButtonState != sugarDecrementLastButtonState) {
    if (sugarDecrementButtonState == LOW) {
      // if the current state is HIGH then the button went from off to on:
      sugarBPress = true;
      sugarButtonPushCounter--;
      if ( sugarButtonPushCounter < 0) sugarButtonPushCounter = 0 ;
      Serial.println("on");
    } else {
      // if the current state is LOW then the button went from on to off:
      Serial.println("off");
  ...
  sugarLastButtonState = sugarButtonState;
  sugarDecrementLastButtonState = sugarDecrementButtonState;
